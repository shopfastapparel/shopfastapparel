...
export const Route = createFileRoute("/group/new-heights-youth/dashboard")({
  head: () => ({
    meta: [
...
function NewHeightsGroupAdminDashboard() {
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      const parsed: ParsedSubmission[] = (data || []).map((row: any) => {
        // Parse items out of notes
        const notesText = row.notes || "";
        const lines = notesText.split("\n");
        const items: ParsedItem[] = [];

        lines.forEach((line: string) => {
          // Format e.g. "1. Option 1: Indigo Sweatshirt (Indigo Blue) — Size: Adult M, Qty: 2"
  ...
  // Calculate volume pricing estimate
  let unitPriceEstimate = 18.00; // Base estimate
      ...
      <div className="mx-auto max-w-7xl px-4 py-10">
        {!authenticated ? (
          ...
          <div className="space-y-10">
            ...
            {/* Individual Member Submissions List */}
            <div className="bg-card border-2 border-ink rounded-xl p-6 md:p-8 shadow-pop">
