...
interface CartLinesAddData {
  data?: {
    cartLinesAdd?: {
      cart?: { lines: { edges: Array<{ node: { id: string; merchandise: { id: string } } }> } };
      userErrors?: Array<{ field: string[] | null; message: string }>;
    };
  };
}
...
async function updateShopifyCartLine(cartId: string, lineId: string, quantity: number) {
  ...
  })) as { data?: { cartLinesUpdate?: { userErrors?: Array<{ field: string[] | null; message: string }> } } } | undefined;
...
async function removeLineFromShopifyCart(cartId: string, lineId: string) {
  const data = (await storefrontApiRequest(CART_LINES_REMOVE_MUTATION, {
    cartId,
    lineIds: [lineId],
  })) as { data?: { cartLinesRemove?: { userErrors?: Array<{ field: string[] | null; message: string }> } } } | undefined;
  ...
  persist(
    (set, get) => ({
      ...
      syncCart: async () => {
        ...
        set({ isSyncing: true });
        try {
          const data = (await storefrontApiRequest(CART_QUERY, { id: cartId })) as
